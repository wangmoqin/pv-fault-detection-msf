...
def parse_opt():
    ...
    parser.add_argument('--data', default=r'./VOCData')  # 数据集路径
    ...
    parser.add_argument('--batch', default=16, type=int, help='Batch size')  # 新增参数 batch
    parser.add_argument('--epochs', default=150, type=int, help='Number of epochs')  # 新增参数 epochs
    parser.add_argument('--imgsz', default=640, type=int, help='Image size')  # 新增参数 imgsz
    parser.add_argument('--device', default='0', type=str, help='CUDA device, i.e. 0 or 0,1,2,3 or cpu')  # 使用GPU或CPU的设备
...
def main(opt):
    ...
    # 从YAML文件加载类名
    classes_file = sorted(dataset_path.rglob('myvoc.yaml'))[0]
...
if __name__ == "__main__":
    ...
    model = YOLO('yolov8l.pt', task='train')
    # 从文本文件中加载内容并存储到一个列表中
    ds_yamls = []
    ...
    for k in range(opt.ksplit):
        ...
        model.train(
            data=dataset_yaml,
            batch=opt.batch,  # 使用命令行传入的 batch 大小
            epochs=opt.epochs,  # 使用命令行传入的 epochs 数量
            imgsz=opt.imgsz,  # 使用命令行传入的图片尺寸
            device=0,
            workers=0,
            project="runs/train",
            name=name,
